...
選択肢: 美容院, メンズサロン, 女性専用サロン, ヘッドスパ専門, 縮毛矯正専門, 髪質改善専門, 総合サロン
判断のヒント：
- 「メンズ」「men's」「男性」が含まれる → メンズサロン
- 「ヘッドスパ」「head spa」が含まれる → ヘッドスパ専門
- 「縮毛」「ストレート」が含まれる → 縮毛矯正専門
- 「髪質改善」「酸熱」「トリートメント専門」が含まれる → 髪質改善専門
...
SURVEY_OPTIONS = {
    ...
    "総合サロン": {
        "questions": [
            {"id": "menu", "text": "今回のメニューは何でしたか？", "type": "multi", "required": True,
             "options": ["カット", "カラー", "パーマ", "縮毛矯正", "髪質改善トリートメント", "酸熱トリートメント", "トリートメント", "ヘッドスパ", "ブリーチ", "インナーカラー", "その他"]},
            ...
            {"id": "good_points", "text": "特によかった点を教えてください", "type": "multi", "required": False,
             "options": ["カラーの発色", "スタイルの提案力", "カットの技術", "グレージュ・透明感の仕上がり",
                         "縮毛矯正の自然さ", "髪のサラサラ感・ツヤ", "ダメージ改善の実感",
                         "ヘアダメージへの配慮", "スタッフの対応", "店内の雰囲気",
                         "丁寧なカウンセリング", "施術時間"]},
    ...
    "女性専用サロン": {
        "questions": [
            ...
            {"id": "good_points", "text": "特によかった点を教えてください", "type": "multi", "required": False,
             "options": ["カラーの発色", "スタイルの提案力", "カットの技術", "グレージュ・透明感の仕上がり",
                         "縮毛矯正の自然さ", "髪のサラサラ感・ツヤ", "ダメージ改善の実感",
                         "ヘアダメージへの配慮", "スタッフの対応", "店内の落ち着いた雰囲気",
                         "丁寧なカウンセリング", "プライベート感"]},
            {"id": "revisit", "text": "また来店したいと思いますか？", "type": "single", "required": True,
             "options": ["ぜひまた来たい", "たぶん来ると思う", "まだわからない"]},
            {"id": "comment", "text": "お気づきの点など（任意）", "type": "text", "required": False,
    ...
    "髪質改善専門": {
        "questions": [
            {"id": "menu", "text": "今回のメニューは何でしたか？", "type": "multi", "required": True,
             "options": ["髪質改善トリートメント", "酸熱トリートメント", "髪質改善ストレート", "縮毛矯正+髪質改善", "カット+髪質改善", "その他"]},
            {"id": "satisfaction", "text": "仕上がりはいかがでしたか？", "type": "single", "required": True,
             "options": ["とても満足", "満足", "普通", "少し不満", "不満"]},
            {"id": "good_points", "text": "特によかった点を教えてください", "type": "multi", "required": False,
             "options": ["髪のサラサラ感", "ツヤの仕上がり", "ダメージの改善実感", "翌日以降の手触り",
                         "カウンセリングの丁寧さ", "ホームケアのアドバイス", "スタッフの技術", "施術時間"]},
            {"id": "revisit", "text": "また来店したいと思いますか？", "type": "single", "required": True,
             "options": ["ぜひまた来たい", "たぶん来ると思う", "まだわからない"]},
            {"id": "comment", "text": "お気づきの点など（任意）", "type": "text", "required": False,
             "placeholder": "仕上がりや髪の変化についてご感想があればご記入ください"},
        ]
    },
    "飲食店": {
        "q1": "総合的な満足度",
...
def submit_feedback(slug):
    ...
    try:
        ...
        system_prompt = f"""あなたの役割は「お客様のアンケート回答を、そのお客様自身の言葉として自然な口コミ文に変換する」ことです。
アンケートに答えてくれた「{shop['name']}（業種: {dict(shop).get('business_type') or '不明'}）」のお客様の体験をそのまま言語化してください。

【最重要ルール】
- アンケートに記載されていない情報（来店きっかけ・人物設定・エピソードなど）を想像・補完しない
- 実際に回答された内容だけを素材にして文章を作る
- アンケートに「また来たい」とある場合のみ再来店の意思を含める
...
【アンケートの使い方】
アンケート項目名をそのまま文章に出すのは禁止。
例：「接客の星が高い」→「担当の方が話しやすくて」のように体験として書く。
...
- 他店との比較表現は一切使わない
- アンケートにない来店きっかけ・人物設定を作らない

ルール絶対遵守: メタ情報や前置きテキストは含めず、純粋な口コミテキストのみを出力してください。"""
        ...
        if b_type in SALON_TYPES:
            ...
            _COMMON_RULES = (
                "【最重要ルール】\n"
                "- アンケートに記載されていない情報（来店きっかけ・人物設定・エピソード）を想像・補完しない\n"
                "- 実際にアンケートで回答された内容だけを素材にして文章を作る\n"
                "- 「また来店したい」とアンケートにある場合のみ再来店の意思を含める\n\n"
                "【文体】以下からランダムに1つ選ぶ：\n"
                "- 丁寧な敬語（〜です・〜ます調）\n"
                "- やや砕けた口語（〜でした、〜だった混在）\n"
                "- 絵文字を1〜2個使う（✂️💇‍♀️✨など）\n"
                ...
                "【アンケートの使い方】\n"
                "アンケート項目名をそのまま文章に出すのは禁止。\n"
                "例：「仕上がりの満足度が高い」→「鏡で見て思ってた以上の仕上がりで」のように体験として書く。\n\n"
                "【絶対禁止ワード】\n"
                "「居心地」「清潔感」「技術が高い」「技術が優れている」「丁寧な接客」「また利用したいと思います」「おすすめです」「癒されました」「リラックスできました」「スタッフの方々」「総合的に」「コスパが良い」「コスパ最高」「また伺います」「大満足」「素晴らしい」「しっかり汲み取って」「SNSで見かけて」「SNSで見つけて」「インスタで」「インスタグラムで」「TikTokで」\n\n"
                "【禁止事項】\n"
                "- アンケートの項目名をそのまま文章に使わない\n"
                "- 「とても」「非常に」「本当に」を連続使用しない\n"
                "- 定型文っぽい締め方をしない\n"
                "- 「ご協力」「アンケート」などのワードを絶対に含めない\n"
                "- 他店・他の美容室・他のサービスとの比較表現は一切使わない\n"
                "- アンケートにない来店きっかけや人物設定を作らない\n\n"
                "口コミ文章のみを出力してください。"
            )
            ...
            _FEW_SHOTS_KAMISHITSU = (
                "【実際の口コミ例（髪質改善・酸熱トリートメント）】\n"
                "以下は髪質改善専門サロンに実際に投稿された口コミです。このリアルさ・自然さを参考にしてください。\n\n"
                "例1: 酸熱トリートメントをお願いしました。今まで広がりやすくてまとまらなかった髪が、施術後は驚くほどサラサラに！翌日も翌々日も手触りが続いていて感動です。\n"
                "例2: 髪質改善トリートメントを初めて体験しました。細かくカウンセリングしてくださって、自分の髪の悩みをしっかり聞いてもらえた上で施術してもらえました。仕上がりのツヤが別人みたいです！\n"
                "例3: 縮毛矯正と髪質改善のセットメニューをしてもらいました。ダメージを最小限に抑えながらストレートにしてもらえて、ドライヤーがすごく楽になりました。丁寧な説明もよかったです。\n"
                "例4: 毎回髪質改善トリートメントをお願いしています。回数を重ねるごとに髪のまとまりが良くなっていて、朝のスタイリング時間が半分になりました！ホームケアの提案も助かります。\n"
                "例5: 初めて酸熱トリートメントを受けました。施術中の熱と独特のにおいはありますが、仕上がりは本当にツルンツルン。美容師さんが丁寧に工程を説明してくれたので安心して受けられました。\n\n"
            )
            _FEW_SHOTS_WOMEN = (
                "【実際の口コミ例（女性専用サロン）】\n"
            ...
            if b_type == "メンズサロン":
                ...
                user_prompt = (
                    "あなたは髪質改善・酸熱トリートメント専門サロンのお客様です。以下のアンケート回答をもとに、"
                    "Googleマップに投稿する自然な口コミ文章を日本語で書いてください。\n\n"
                    + _SURVEY +
                    "【口コミ作成のルール】\n"
                    "- 以下の文字数パターンからランダムに1つ選んで生成すること：\n"
                    ...
                    "  ・140〜180文字（20%の確率）\n"
                    "  選んだ範囲内に必ず収めること\n"
                    "- 髪質改善ならではの体験（サラサラ感、ツヤ、ダメージレス、翌日の手触り、ホームケアアドバイスなど）を具体的に表現する\n"
                    "- 満足度が「普通」以下の場合は星4以下を示唆する表現にする\n"
                    "- 「また来たい」場合は再来店の意思を含める\n"
                    "- 箇条書きや記号は使わない\n"
                    "- 冒頭に「サンプル」などのテスト感のある言葉は使わない\n\n"
                    + _COMMON_RULES + _FEW_SHOTS_KAMISHITSU
                )
            elif b_type == "女性専用サロン":
                user_prompt = (
