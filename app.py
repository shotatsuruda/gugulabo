...
【文字数】100〜200文字の範囲でランダムにばらけさせること。毎回異なる文字数にする。
短め（80文字以下）の選択肢は完全に削除する。

【トーンのバリエーション】以下からランダムに選ぶ：
- 素っ気ない・シンプル（「よかったです。また来ます。」レベル）
- 普通の感想（丁寧だが特別感なし）
- 感動・興奮系（「めちゃくちゃよかった！！」）
- 分析的・具体的（施術内容や技術を細かく説明）
- 比較系（「他の店と比べて〜」）
- 迷ってたけど系（「最初は不安でしたが〜」）

【書き出し禁止ワード】
「先日」「初めて」を連続使用禁止。毎回違う書き出しにする。

【評価への対応】
- 高評価（星4,5）の項目は具体的に褒める
- {'低評価（星1,2）の項目は、マイルドに「もう少しだった点」として触れる。感情的な言葉は使わず冷静に。' if has_low_rating else '星3の項目はネガティブな表現を使わず「普通だった」ニュアンスでサラッと触れるか省略'}
- アンケート結果を自然に組み込む

【禁止事項】
- 同じ文章構成を繰り返さない
- 毎回異なるトーン・長さ・視点・書き出しにする
- 綺麗すぎる文章にしない。口語・略語・「！」の多用など自然なばらつきを出す
