...
def submit_feedback(slug):
    ...
    try:
        ...
        system_prompt += "【ルール】\n"
        system_prompt += "1. 指定されたアンケート結果（各項目の評価、コメント）を自然に組み込んでください。特に評価が高い項目（星4,5）は具体的に褒めてください。\n"
        
        # Check if any rating is 1 or 2
        has_low_rating = any(r and int(r) <= 2 for r in [rating, rating2, rating3, rating4, rating5])
        
        if has_low_rating:
            system_prompt += "2. 評価が低い項目（星1,2）については、マイルドに「もう少しだった点」や「気になった点」として触れてください。\n"
            system_prompt += "3. 定型文のような「とてもよかったです」だけの文章は禁止。他の人の参考になるような具体的なエピソード風に書いてください。\n"
            system_prompt += "4. 大げさなサクラのような表現は避け、自然な話し言葉で書いてください。\n"
            system_prompt += "5. 総合評価が低い場合や低い項目がある場合は、感情的な言葉を使わず「建設的な要望や、気になった点」を冷静に伝える誠実なレビューにしてください。\n"
        else:
            system_prompt += "2. 評価が星3の場合は、ネガティブな表現を使わず「普通だった」というニュアンスでサラッと触れるか、触れなくても構いません。\n"
            system_prompt += "3. 定型文のような「とてもよかったです」だけの文章は禁止。他の人の参考になるような具体的なエピソード風に書いてください。\n"
            system_prompt += "4. 大げさなサクラのような表現は避け、自然な話し言葉で書いてください。\n"
        
        system_prompt += "【書き出しのルール】\n"
        system_prompt += "- 「先日」「初めて」を連続して使わないこと。\n"
        system_prompt += "- 以下の書き出しパターンからランダムに選ぶこと：「通い始めてから」「〇回目の来院ですが」「久しぶりに」「友人の紹介で」「口コミを見て」「近所なので」「仕事帰りに」「気になっていたので」「思い切って」\n"
        system_prompt += "- 同じ症状（腰・肩など）が続かないよう、症状もバリエーションをつけること。\n"
        system_prompt += "- 毎回まったく異なるトーン・視点・構成で書くこと。\n"
